...
class BookProduct implements ProductInterface
{
    /**
     * @var array $inputs
     */
    ...
    public function __construct($inputs)
    {
        if (!isset($inputs['weight'])){
            die(json_encode(array(
                'error'=> 'weight is required for Book product'
            )));
        }
        else{
            $this->inputs = $inputs;
    ...
    public function validateInputs()
    {
        ...
        $weightErrors = $Validator->name('weight')->value($this->inputs['weight'])->required()->isNumber()->getErrors();
        if(!empty($weightErrors)) $this->errors['weight'] = $weightErrors;
    ...
    public function printProduct()
    {
        $element = array(
            ...
            'attribute' => $this->inputs["weight"] ." KG"
        );
    ...
    public function insertIntoDb():bool
    {
        if(empty($this->errors)){
            ...
            $query->addValue($this->inputs['weight']." KG");
