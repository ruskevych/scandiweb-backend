...
class DiskProduct implements ProductInterface
{
    ...
    public function __construct($inputs)
    {
        if (!isset($inputs['size'])){
            die(json_encode(array(
                'error'=> 'size is required for Book product'
            )));
        }
        else{
            $this->inputs = $inputs;
    ...
    public function validateInputs()
    {
        ...
        $priceErrors = $Validator->name('price')->value($this->inputs['price'])->required()->getErrors();
    ...
    public function insertIntoDb():bool
    {
        if(empty($this->errors)){
            ...
            $query->addValue($this->inputs['size']." MB");
